...
const int relay1pin = 25;  // the number of the LED pin
const int relay2pin = 33;  // the number of the LED pin
const int relay3pin = 32;  // the number of the LED pin
...
static int transInt = 2000;
static int lastdet = 0;
static int detectionongoing = 0;
static int firstimeflag = 0;
...
void receivedCallback(uint32_t from, String &msg)
{
  ...
  if( (Temp1 != "null") || (Hum1 != "null") || (Lux1 != "null")  )
  {
    ...
    if(Motion1.toInt() < motion_threshold)
    {
  ...
  if( (Temp2 != "null") || (Hum2 != "null") || (Lux2 != "null") )
  {
    ...
    if(Motion2 != "null") 
    {
      ...
      if(Motion2.toInt() < motion_threshold)
      {
  ...
  Serial.printf("d1:%d d2: %d \n",detection1_set,detection2_set);

  if((detection1_set == 1) && (detection2_set == 0))
  {
    incrementlogic = 1;
  }
  ...
  if((detection2_set == 1) && (detection1_set ==0))
  {
    decrementlogic = 1;
  }
  ...
  if(decrementlogic == 1)
  {
    if((detection1_set == 1)&&(detection1_pervious == 0))
    {
      if(PersonCount > 0)
      PersonCount --;
      detectionongoing = 0;
  ...
  if(detection1_set==1 && detection2_set==1)
  { 
    Serial.printf("c:%d l: %d diff %d\n",millis(),lastdet,(millis() - lastdet));

    if(detectionongoing == 0)
    {
      lastdet = millis();
    }
    detectionongoing = 1;
    ...
    {
        if(motion1 > motion_threshold && motion2 > motion_threshold)
        {
          detection1_set = 0;
          detection2_set = 0;
          incrementlogic = 0;
  ...
  detection1_pervious = detection1_set;
